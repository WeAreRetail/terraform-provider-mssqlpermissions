...
var (
	_ datasource.DataSource              = &schemaPermissionsDataSource{}
	_ datasource.DataSourceWithConfigure = &schemaPermissionsDataSource{}
)
...
// Metadata sets the metadata for the schema permissions data source.
func (d *schemaPermissionsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_schema_permissions"
}

// Schema defines the schema for the schema permissions data source.
func (d *schemaPermissionsDataSource) Schema(_ context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description:         "Reads schema-level permissions assigned to a role.",
		MarkdownDescription: "Reads schema-level permissions assigned to a role.",
		Attributes: map[string]schema.Attribute{
			...
			"permissions": schema.ListNestedAttribute{
				Description:         "List of permissions assigned to this role on the schema.",
				MarkdownDescription: "List of permissions assigned to this role on the schema.",
				Computed:            true,
...
func (d *schemaPermissionsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *schemaPermissionsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading schema permissions for role", map[string]interface{}{
		"schema_name": data.SchemaName.ValueString(),
	...
	// Get permissions for the role on the schema
	permissions, err := connector.GetSchemaPermissionsForRole(ctx, db, role, schemaName)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error Reading Schema Permissions",
			"Could not read permissions for role "+data.RoleName.ValueString()+" on schema "+schemaName+": "+err.Error(),
		)
		return
	}
	...
	tflog.Debug(ctx, "Successfully read schema permissions", map[string]interface{}{
